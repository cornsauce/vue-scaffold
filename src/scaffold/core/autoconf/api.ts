...
import {Configurator, BEAN_REGISTERED} from '@/scaffold/core/application';
...
export function configureAPI(rootConfig: App.Config): Configurator {
  return (app) => {
    ...
    app.registerBean(BeanConstant.API_AXIOS, axios);
    app.registerBean(BeanConstant.API, app.getContainer().resolve(rootConfig.api.class));

    app.getEventEmitter().addListener(BEAN_REGISTERED, ({bean, value}) => {
      if (bean === BeanConstant.AUTH_MANAGER) {
        Log.d(bean, value);
        if (
          ...
          const axios = app.getContainer().get<AxiosInstance>(BeanConstant.API_AXIOS);
